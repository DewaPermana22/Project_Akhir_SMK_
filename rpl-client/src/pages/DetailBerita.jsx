import { ArrowBigLeftDashIcon, Calendar, Eye, Share2, Clock, User, Tag, Heart, MessageCircle } from 'lucide-react'
    ...
    return (
      <main className="min-h-screen bg-[var(--indigo-dark)]">
        <div className='h-[80px] fixed top-0 left-0 right-0 bg-[var(--indigo-dark)] z-50'>
          <Link to={"/"} className='flex items-center gap-2 bg-[var(--lime)] px-5 py-2 rounded-md shadow-2xs cursor-pointer hover:bg-[var(--lavender)] transition-colors ease-linear duration-200 absolute top-5 left-5'>
      ...
      <div className='h-[80px] fixed top-0 left-0 right-0 bg-[var(--indigo-dark)] z-50 border-b border-white/10'>
        ...
        <div className="absolute top-5 right-5 flex gap-3">
          ...
          >
            ...
            <span className="text-sm">{detailBerita?.likes + (liked ? 1 : 0)}</span>
          </button>
          <div className="relative">
            ...
            {showShareMenu && (
              <div className="absolute top-12 right-0 bg-white rounded-lg shadow-xl p-3 min-w-[200px] z-10">
                ...
                <a
                  href={`https://twitter.com/intent/tweet?text=${encodeURIComponent(detailBerita?.judul)}&url=${encodeURIComponent(window.location.href)}`}
                  target="_blank"
        ...
        <article className="max-w-4xl mx-auto">
          <header className="mb-8">
            ...
            <div className="flex items-center gap-4 p-4 bg-white/5 backdrop-blur-sm rounded-xl border border-white/10">
              <img
              loading='lazy'
                src={userDetail?.avatar? userDetail.avatar : "https://ui-avatars.com/api/?name=" + userDetail?.name + "&background=random&size=128&color=fff"} 
                alt={userDetail?.name}
              ...
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-white font-semibold">{userDetail?.name? userDetail.name : "Anonymous"}</span>
                </div>
          ...
          <footer className="mt-12 pt-8 border-t border-white/20">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              ...
              <div className="flex items-center gap-4">
                ...
                >
                  <Heart className={`w-4 h-4 ${liked ? 'fill-current' : ''}`} />
                  <span>{detailBerita?.likes + (liked ? 1 : 0)} likes</span>
                </button>
              </div>
            </div>
          </footer>
