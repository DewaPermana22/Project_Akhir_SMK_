...
const SelectDropdown = ({
  ...
  return (
    <div className={`relative w-full ${size} ${className}`}>
      ...
      >
        <span className={`text-sm font-medium ${selectedValue ? 'text-gray-700' : 'text-gray-400'}`}>
        ...
        <div className="absolute top-full mt-2 w-full bg-white/90 backdrop-blur-sm rounded-xl border border-gray-200 overflow-hidden z-10 animate-in slide-in-from-top-2 duration-200">
          {options.map((option) => (
            <button
              ...
              className="w-full p-4 text-left text-gray-700 cursor-pointer hover:bg-gradient-to-r hover:from-blue-50 hover:to-indigo-50 transition-all duration-200 border-b border-gray-100/50 last:border-b-0 text-sm"
            >
              {option.label}
            </button>
          ))}
